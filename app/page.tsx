...
export default function MemoryApp() {
  ...
  const handleShare = async () => {
    ...
    const level = getLevel(stats.xp)
    const acc = getAccuracy(stats.attempts)
    const shareText = `🧠 I just completed my daily brain training on MemoryGym!\n\n🔥 ${currentStreak}-day streak\n⭐ Level ${level} · ${getLevelTitle(level)}\n${acc !== null ? `🎯 ${acc}% accuracy\n` : ''}💪 ${stats.attempts.length} exercises completed\n\nTrain your brain daily — it only takes 10 minutes!\n👉 https://memory-gym-eight.vercel.app`

    if (navigator.share) {
      try {
        await navigator.share({ title: 'MemoryGym — Brain Training', text: shareText, url: 'https://memory-gym-eight.vercel.app' })
        return
      } catch (e) {}
    }
        ...
        {shareImg && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} onClick={e => e.stopPropagation()}
              className="glass-strong rounded-3xl p-6 max-w-sm w-full space-y-4 text-center">
              {shareImg === 'copied' ? (
                <>
                  <div className="w-16 h-16 bg-green-500/20 rounded-full flex items-center justify-center mx-auto">
                    <CheckCircle2 className="w-8 h-8 text-green-400" />
                  </div>
                  <h3 className="font-display font-bold text-white text-lg">Copied to Clipboard!</h3>
                  <p className="text-slate-400 text-sm">Your streak message is ready to paste into any app — text, email, or social media.</p>
                </>
              ) : (
                <>
                  ...
                  <h3 className="font-display font-bold text-white text-lg">Share MemoryGym</h3>
                  <p className="text-slate-400 text-sm">Share this link with friends and family:</p>
                  <div className="bg-white/10 rounded-xl p-3 text-blue-300 text-sm break-all">
                    https://memory-gym-eight.vercel.app
                  </div>
                </>
              )}
              <Button onClick={() => setShareImg(null)} className="w-full bg-blue-600 hover:bg-blue-500 text-white rounded-xl">
                Done
              </Button>
            </motion.div>
          </motion.div>
